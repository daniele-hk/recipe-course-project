...
// For the current versione eliminate the follow line
    const alertCmpFactory = this.componentFactoryResolver.resolveComponentFactory(AlertComponent);
    const hostViewContainerRef = this.alertHost.viewContainerRef;
    hostViewContainerRef.clear();
// For the current versione eliminate the follow line
    const componenetRef = hostViewContainerRef.createComponent(alertCmpFactory);
// Since Angular 13, the ComponentFactoryResolver is no longer required.
// It doesn't have to be imported, nor does the factory have to be created.
